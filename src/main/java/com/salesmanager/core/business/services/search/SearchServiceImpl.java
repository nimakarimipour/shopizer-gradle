...
public class SearchServiceImpl
  ...
  private static final String KEYWORDS_MAPPING_DEFAULT =
      "{\"properties\":"
          + "      {\n\"id\": {\n"
          + "        \"type\": \"long\"\n"
          + "      }\n"
  ...
  @Autowired private ApplicationSearchConfiguration applicationSearchConfiguration;
  ...
  public void index(MerchantStore store, Product product) throws ServiceException {
    ...
    try {
      ...
      if (!CollectionUtils.isEmpty(product.getVariants())) {
        ...
        variants =
            product.getVariants().stream().map(i -> variants(i)).collect(Collectors.toList());
      }
  ...
  private List<Document> document(Long id, List<String> languages, RequestOptions options)
      throws Exception {
  ...
  private void indexProduct(
    ...
    try {
      ...
      if (!CollectionUtils.isEmpty(product.getCategories())) {
        item.setCategory(
            category(
                product.getCategories().iterator().next(), description.getLanguage().getCode()));
      }
  ...
  private Map<String, String> inventory(Product product) throws Exception {
    ...
    if (inventory.getPrice().isDiscounted()) {
      inventoryMap.put(
          DISCOUNT_PRICE, String.valueOf(inventory.getPrice().getStringDiscountedPrice()));
    }
  ...
  @Override
  public SearchResponse searchKeywords(
      MerchantStore store, String language, SearchRequest search, int entriesCount)
      throws ServiceException {
  ...
  public SearchResponse search(
      MerchantStore store, String language, SearchRequest search, int entriesCount, int startIndex)
      throws ServiceException {
  ...
  private List<String> languages(Product product) {
    return product.getDescriptions().stream()
  ...
  private String manufacturer(Manufacturer manufacturer, String language) {
    ManufacturerDescription description =
        manufacturer.getDescriptions().stream()
            .filter(d -> d.getLanguage().getCode().equals(language))
            .findFirst()
            .get();
  ...
  private Map<String, String> attribute(ProductAttribute attribute, String language) {
    ...
    ProductOptionDescription optionDescription =
        attribute.getProductOption().getDescriptions().stream()
            .filter(a -> a.getLanguage().getCode().equals(language))
            .findFirst()
            .get();

    ProductOptionValueDescription value =
        attribute.getProductOptionValue().getDescriptions().stream()
            .filter(a -> a.getLanguage().getCode().equals(language))
            .findFirst()
            .get();
  ...
  private void mappings(SearchConfiguration config, String language) throws Exception {
    ...
    config
        .getProductMappings()
        .put(language, resourceAsText(loadSearchConfig(PRODUCT_MAPPING_DEFAULT)));
  ...
  private String resourceAsText(Resource resource) throws Exception {
    ...
    return new BufferedReader(new InputStreamReader(mappingstream, StandardCharsets.UTF_8))
        .lines()
