...
public class TransactionServiceImpl extends SalesManagerEntityServiceImpl<Long, Transaction>
   ...
   *
   * <p>Check transactions next transaction flow is Build map of transactions map filter get last
   * from date get last transaction type verify which step transaction it if check if target
   * transaction is in transaction map we are in trouble...
   */
