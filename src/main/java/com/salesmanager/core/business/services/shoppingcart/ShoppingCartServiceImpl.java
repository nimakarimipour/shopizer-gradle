...
public class ShoppingCartServiceImpl extends SalesManagerEntityServiceImpl<Long, ShoppingCart>
  ...
  private void getPopulatedItem(final ShoppingCartItem item, MerchantStore store) throws Exception {
    ...
    List<ShoppingCartAttributeItem> removeAttributesList =
        new ArrayList<ShoppingCartAttributeItem>(); // attributes
    // to remove
  ...
  private Set<ShoppingCartItem> getShoppingCartItems(
    ...
    if (CollectionUtils.isNotEmpty(sessionCart.getLineItems())) {
      ...
      for (ShoppingCartItem shoppingCartItem : sessionCart.getLineItems()) {
        Product product =
            productService.getBySku(shoppingCartItem.getSku(), store, store.getDefaultLanguage());
        ...
        if (shoppingCartItem != null
            && !CollectionUtils.isEmpty(shoppingCartItem.getAttributes())) {
