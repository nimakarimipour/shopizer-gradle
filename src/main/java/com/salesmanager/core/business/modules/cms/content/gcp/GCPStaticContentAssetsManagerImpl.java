...
public class GCPStaticContentAssetsManagerImpl implements ContentAssetsManager {
  ...
  public OutputContentFile getFile(
    ...
    try {
      ...
      Blob blob =
          storage.get(
              BlobId.of(bucketName, nodePath(merchantStoreCode, fileContentType) + contentName));
  ...
  public List<String> getFileNames(
    ...
    try {
      ...
      Page<Blob> blobs =
          bucket.list(
              Storage.BlobListOption.prefix(nodePath(merchantStoreCode, fileContentType)),
              Storage.BlobListOption.fields(BlobField.NAME));

      List<String> fileNames = new ArrayList<String>();

      for (Blob blob : blobs.iterateAll()) {
        if (isInsideSubFolder(blob.getName())) continue;
